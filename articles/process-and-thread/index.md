...
この図は「2 つのプロセスが存在する状態」を示している。プロセスA 内のスレッド 1 とスレッド 2 は共有領域（ヒープ・グローバル変数）を持ち合う一方、プロセスB の専用領域とは完全に隔離されている。見るポイントは「同じプロセス内ではメモリが共有される」「プロセス間は原則隔離」の 2 点だ。
...
> #### 💡 ワンポイントアドバイス
> - 「確認してから操作する 2 ステップ」が競合状態の本質。コードを読むとき「この 2 ステップを 1 ステップにできないか？」を意識するだけで脆弱な実装に気づきやすくなる。
> - Node.js の `await` は「一時停止」ではなく「別のリクエストへの切り替え許可」と考えると競合状態の原因が見えやすい。`await` の前後は必ずアトミック性を確認しよう。
> - Mutex は `with` 構文や `try...finally` と常にセットで使う。解放忘れがデッドロックの主な原因だ。

### 攻撃フロー — 競合状態による残高操作

この例では初期残高を 1000 円、同時出金額を 800 円として競合状態を表現している。図では「2 つのリクエストが競合してロックなしの DB から二重出金を成立させる経路（競合成功）」と「トランザクションのロックで片方が弾かれる経路（ロック取得失敗）」の両方を示している。
...
**問題点（TOCTOU）**: `file_exists()` で「存在しない」と確認してから `file_put_contents()` で書き込むまでの間に、別のリクエストが同じファイル名で書き込む可能性がある。確認と書き込みが 2 つの独立したステップになっているため、その隙間に割り込める。
...
**問題点**: `balance` を読み取った直後に `await` があるため、2 つの `withdraw()` 呼び出しが同じ `balance = 1000` を読んだ後にそれぞれ引き落とす。両方が「残高十分あり」と判定してしまうため、合計 1600 円の出金が成立する。
...
**問題点**: ①で読み取った `current` の値が、②の待機中に別スレッドによって更新されても、③は古い `current` を使って書き込む。この「読み取り → 書き込み」の間の割り込みが競合状態の本質だ。実行するたびにスケジューラの動作が変わるため、結果（実際のカウント）も毎回変化する。
...
**誤解 1: 「Node.js はシングルスレッドだから競合状態は起きない」**
...
**誤解 3: 「Mutex を使えば競合状態はすべて解決する」**
Mutex は正しく使えば有効だが、使い忘れ・ロック粒度の誤り・デッドロックが起きるリスクがある。DB を使う場合はトランザクション（`FOR UPDATE` など）と組み合わせるか、アトミックな SQL 文で一括処理する方が確実だ。

**誤解 4: 「競合状態は理論上の問題でリアルな攻撃は難しい」**
Burp Suite の「Turbo Intruder」や Python の `threading` モジュールを使えば、数十ミリ秒以内に数百リクエストを並列送信できる。実際に CVE になった競合状態（後述）の多くは現実の攻撃として悪用されている。
