...
{/* Mermaid 追加: 数値表現の種類と相互変換の関係を示す flowchart */}
```mermaid
flowchart TD
...
{/* 修正: 「0x が付けば hex と思えば良い」は不十分。xxd/Wireshark ではプレフィックスなしが一般的なため補足を追加 */}
...
{/* 修正: 右シフトが「x ÷ 2^n と等価」は符号付き整数・言語仕様によって異なるため限定条件を追記 */}
...
{/* 修正: 「パーミッションのチェックは内部で AND 演算している」は単純化しすぎ。Linux カーネルは ACL・Capabilities 等も考慮するため補足を追加 */}
...
{/* Mermaid 追加: PHP マジックハッシュによるログインバイパスの攻撃フロー */}
```mermaid
sequenceDiagram
...
// {/* 修正: コメントに固定の hex 値を断定記載すると実行結果と乖離して読者が混乱するため削除。自分の環境で実行して確認すること */}
...
# {/* 修正: 「整数オーバーフローを考慮していない」は技術的に不正確。
#            Python の int は任意精度でオーバーフローしない。
#            実際の問題は「未検証のビットフラグ入力をそのまま権限判定に使っている」点のため修正 */}
# ─────────────────────────────────────────────
...
# {/* 修正: XOR クラッキングは L1 範囲を超えるため「発展」として明示 */}
# ─────────────────────────────────────────────
...
{/* 修正: 日本法のみへの依存を解消し、海外読者向けに各国法令・利用規約への準拠も明記 */}
...
{/* 修正: 「0x45 = Ethernet フレームの場合は IPv4 ヘッダーの先頭」は誤り。
           0x45 は Ethernet ヘッダーの後ろに続く IPv4 ヘッダーの先頭バイト */}
...
│
└───────── Version=4, IHL=5
           {/* 修正: 「(4+1=5 → ×4)」は誤り。IHL は 32bit ワード数であり Version と加算しない */}
...
{/* 修正: 「パスワードは bcrypt」は 2026 年時点では古い推奨。OWASP の現行推奨は Argon2id が優先 */}
| 用途 | 推奨 |
|------|------|
| 対称暗号 | AES-256-GCM |
...
{/* 修正: 右シフトと除算の等価性は「非負整数または論理右シフト」に限定して説明 */}
### 誤解 3: 「シフト演算と乗除算は別物だ」
