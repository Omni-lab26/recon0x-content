...
flowchart TD
    A[10進数\n例: 180] -->|÷2 を繰り返す| B[2進数\n例: 10110100]
    B -->|4ビットずつ区切る| C[16進数\n例: 0xB4]
    C -->|プレフィックス 0x| D[プログラム・ダンプ表記]
    B -->|8ビット = 1バイト| E[バイト列\nメモリ・パケット]
    E -->|エンディアン変換| F[ビッグエンディアン\nネットワーク標準]
    E -->|エンディアン変換| G[リトルエンディアン\nx86 CPU]
```
...
<!-- 修正: 「0x が付けば 16 進数」だけでは不十分。xxd 等のダンプではプレフィックスなし hex が一般的なため補足を追加 -->
`0x` プレフィックスが付いていれば確実に 16 進数だが、Wireshark や `xxd` の出力ではプレフィックスなしで hex が並ぶことが多い。文脈と桁数（2 桁 = 1 バイト）で判断しよう。
...
<!-- 修正: BOF への過度な言及を削減し「後続記事で扱う」と明示 -->
エンディアンの知識はパケット解析やメモリダンプを読む際にすぐ役立つ。バッファオーバーフローへの応用（リターンアドレスのバイト順など）は後続の「バッファオーバーフロー入門」で詳しく扱う。
...
flowchart TD
    ...
    AND -->|両方 1 のとき 1| A1[ネットマスク計算\nフラグのチェック]
    OR  -->|どちらか 1 なら 1| O1[フラグのセット\n権限ビット追加]
    XOR -->|異なるとき 1| X1[簡易暗号化\nCTF 頻出\nハッシュ内部]
    NOT -->|全ビット反転| N1[マスクの反転\n補数計算]
    SHIFT -->|2のべき乗 乗除| S1[高速乗除算\nパケットフィールド抽出]
```
...
<!-- 修正: 右シフトの「÷2^n と等価」が符号付き整数では成立しないケースがあるため限定条件を追記 -->
シフト演算は非負整数（または論理右シフト）の場合、`x << n` は `x × 2^n`、`x >> n` は `x ÷ 2^n` と数学的に等価だ。ただし符号付き整数の右シフト挙動は言語・処理系によって異なる点に注意しよう。コンパイラは整数の 2 のべき乗乗算を自動的にシフトに最適化することが多い。
...
<!-- 修正: 「パーミッションのチェックは AND 演算している」が単純化しすぎのため、ACL 等も考慮する旨の補足を追加 -->
...
sequenceDiagram
    ...
    攻撃者->>PHPアプリ: パスワード "QNKCDZO" を送信
    PHPアプリ->>PHPアプリ: md5("QNKCDZO") → "0e830400..."
    PHPアプリ->>DB: stored_hash を取得
    DB-->>PHPアプリ: "0e462097..." (正規ユーザーのハッシュ)
    PHPアプリ->>PHPアプリ: "0e830..." == "0e462..." → PHP が両方を 0 と評価 → true
    PHPアプリ-->>攻撃者: ログイン成功（バイパス成立）
```
...
// 実行結果: 環境によって異なる場合があるため、node で実際に確認してください
// 例: node -e "const e=Buffer.from('Hello, recon0x!').map(b=>b^'K'.charCodeAt(0)); console.log(e.toString('hex'))"
// <!-- 修正: 元のhex出力コメントは実際の実行結果と一致しなかったため削除。読者の実行環境で確認するよう案内に変更 -->
...
# <!-- 修正: 「整数オーバーフローを考慮していない」は不正確。実際の問題は
#            「未検証のビットフラグ入力をそのまま権限判定に使っている」点のため修正 -->
# ─────────────────────────────────────────────
...
def check_permission_vulnerable(user_flags: int, required: int) -> bool:
    """
    脆弱なチェック: 外部から受け取った user_flags を検証せずそのまま使う例。
    <!-- 修正: Python の int は任意精度なので「符号付き整数オーバーフロー」は発生しない。
               問題は未検証の入力値 -1 が AND 演算で全権限を満たしてしまう点 -->
    例えば API 経由で -1 を渡すと、Python の任意精度 int では全ビットが立った状態になり、
    あらゆる権限チェックを通過してしまう。
    """
    return bool(user_flags & required)  # ← user_flags の検証がない

# 攻撃例: 入力値 -1 はすべてのビットが 1（Python の ~0 と同等）
...
def crack_single_byte_xor(ciphertext: bytes) -> tuple[int, str]:
    """
    単一バイト XOR 暗号を英文スコアで解読する
    CTF の最頻出パターンの 1 つ（発展的内容）
    """
    # 英語の頻度が高い文字（スコアリング用）
...
def demo_conversions():
    ...
    # エンディアンの違いはパケット解析・メモリダンプで重要
    # バッファオーバーフローへの応用は後続の「Binary Exploitation」記事で扱う
...
if __name__ == "__main__":
    ...
    print(f"\nXOR解読結果 → key: 0x{key:02x} ({chr(key)}), plain: {plain}")
```
...
<!-- 修正: 不正アクセス禁止法への言及に「日本法のみ」という限定を補い、海外読者向けに各国法令・利用規約への準拠も明記 -->
> ⚠️ **注意**: 以下の演習はすべて自分が所有・管理する環境、または HTB / TryHackMe のような **利用規約に明示された合法ラボ** のみで実施すること。許可のない第三者システムへの適用は **不正アクセス禁止法（第 3 条）** に違反する（日本法）。海外在住の読者は各国の法令およびサービスの利用規約にも従うこと。
...
<!-- 修正: 「0x45 = Ethernet フレームの場合は IPv4 ヘッダーの先頭」は誤り。
           0x45 は Ethernet ヘッダーの後ろに続く IPv4 ヘッダーの先頭バイトであるため修正 -->
4. `0x45` は **Ethernet ヘッダーの後ろに現れる IPv4 ヘッダー先頭バイト**（Version=4, IHL=5）
...
│
└───────── Version=4, IHL=5
           <!-- 修正: 「(4+1=5 → ×4)」は意味が通らない誤記。IHL は32bitワード数なので ×4 で修正 -->
           IHL=5 → 5 × 4 = 20 バイトのヘッダー長
```

### 演習 3: HTB CTF での XOR 解読（発展）

<!-- 修正: XOR クラッキングは L1 の本文より発展的な内容のため「発展」と明示 -->
...
<!-- 修正: 「パスワードは bcrypt」が2026年時点では古い推奨のため、OWASP の現行推奨に合わせて Argon2id を優先と明記 -->
| 用途 | 推奨 |
|------|------|
| 対称暗号 | AES-256-GCM |
...
**実際**: 非負整数に対して `x << n` は `x × 2^n`、`x >> n` は `x ÷ 2^n` と数学的に等価。コンパイラは整数の 2 のべき乗乗算を自動的にシフトに最適化する。ただし符号付き整数の右シフトは言語・処理系によって挙動が異なるため（算術シフト vs 論理シフト）、依存したコードは移植性に注意が必要だ。

### 誤解 4: 「エンディアンは低レイヤの話で CTF には関係ない」

**実際**: パケット解析でもメモリダンプでも即座に登場する概念だ。pwn 系 CTF でバッファオーバーフローを扱う際のリターンアドレス操作など、より発展的な応用は後続の「バッファオーバーフロー入門」で取り上げる。
...
- **[バッファオーバーフロー入門: スタックの仕組み]** — リトルエンディアン・アドレス操作の応用（本記事の発展）
