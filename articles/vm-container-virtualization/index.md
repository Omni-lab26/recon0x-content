...
クラウドネイティブ時代のインフラはほぼコンテナで動いている。コンテナのセキュリティを理解しないと、以下の問いに答えられない。

- **なぜ `--privileged` コンテナはすぐにホストを乗っ取れるのか？** → namespace の分離が事実上解除されるから。
- **コンテナ内で root になれてもなぜ安全なのか？** → user namespace が有効なら「コンテナ内の root」はホストの一般ユーザー相当だから。ただし設定次第で逆転する。
- **Docker ソケットをマウントするとなぜ危険なのか？** → Docker デーモンへの接続権限 = ホスト上で任意コンテナを起動できる権限だから。
...
この図は VM（ハイパーバイザー型）とコンテナ型の層構造の違いを示している。VM はゲスト OS カーネルを持つが、コンテナはホストカーネルを直接共有する点が本質的な違いだ。
...
この図は Linux namespace の 6 種類と、各々が分離するリソースの対応を示している。
...
この図は「攻撃者がコンテナ内に侵入してからホストへエスケープを試みる流れ」と「各段階で防御が機能した場合の阻止経路」を示している。
...
**問題点**: `?image=` の値を検証せず Docker API に渡している。攻撃者が任意のイメージを指定でき、加えて `/:/host` という危険なバインドマウントがハードコードされているため、コンテナを起動した時点でホスト全体が読み書き可能になる。
...
**問題点**: `/debug` エンドポイントが環境変数全体を返す。Docker コンテナで `-e DATABASE_PASSWORD=secret123` のように渡した秘密情報が HTTP で取得できる。
...
公開して良い環境変数キーをホワイトリストで明示し、シークレット系（`PASSWORD`・`SECRET`・`KEY`・`TOKEN` を含むキー）は絶対に含めない。
...
**問題点**: `size` の値に制限がなく、攻撃者が大きな値を送るとコンテナが OOM Kill で落ちる（DoS）。また cgroups のメモリ制限が設定されていないとホストのメモリを食い尽くす。
...
**誤解 1: 「コンテナ内の root は安全」**
user namespace が有効なら「コンテナ内の root = ホストの一般ユーザー」だが、user namespace を使わないデフォルト Docker では、カーネル内部でコンテナ内の root は UID 0 として扱われる。namespace や capability による制限は存在するが、コンテナエスケープに成功した場合のリスクは大きい。`--userns-remap` を設定することでエスケープ後の影響を大幅に限定できる。

**誤解 2: 「Docker は VM と同じくらい安全」**
...
**誤解 3: 「`docker` グループに追加するのは sudo を避けるための良い方法」**
`docker` グループのメンバーは `sudo` なしで Docker を操作できるが、`docker run -v /:/host -it ubuntu chroot /host bash` で即座にホスト root になれる。`docker` グループへの追加 = root 権限の付与と同義だ。

**誤解 4: 「プライベートレジストリのイメージは安全」**
プライベートレジストリに置いたイメージでも、ビルドに使ったベースイメージや依存パッケージに脆弱性があれば危険だ。`trivy` や `grype` で定期的にスキャンし、ベースイメージを最新に保つことが必要。

**誤解 5: 「環境変数はプロセス内にあるから外から見えない」**
コンテナ内では `/proc/[PID]/environ` を読める権限があれば環境変数を確認できる。また `docker inspect` でホスト側から全環境変数が見える。秘密情報は環境変数ではなく Docker Secrets や Vault 等の秘密管理システムに置くべきだ。
