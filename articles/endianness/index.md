...
「数値 `0x12345678` をメモリに書く」という単純な操作でも、エンディアンが異なると全く別のバイト列になる。これが問題になる場面は広い。

- **CTF Pwn / エクスプロイト開発**: x86 の Return Address `0xdeadbeef` をスタックに書くとき、リトルエンディアンの順番（`\xef\xbe\xad\xde`）で送らなければ exploit は動かない。
...
この図は「エンディアン」の 2 種類とそれぞれを採用しているアーキテクチャの関係を示している。**見るポイント**: MSB と LSB の配置順の違い・どのアーキテクチャがどちらを採用しているか・ネットワーク通信との関係だ。
...
この図は「ネットワーク（ビッグエンディアン）で送られてきた 32 ビット長さフィールドを、リトルエンディアンのサーバーが変換せずに読んだ場合の大きさ誤認」と「`ntohl` 変換と長さチェックで正しく処理される場合」を示している。
...
**問題点**: ネットワークから来るデータはビッグエンディアンが標準だ。`unpack('V', ...)` でリトルエンディアンとして読むと、`0x00 0x00 0x01 0x00`（正しくは 256）が `0x00010000 = 65,536` と解釈される。`fread($socket, 65536)` は想定外に大きな読み取り要求となり、メモリ消費や待機時間増大を招く。
...
`unpack('N', ...)` でビッグエンディアン（ネットワークバイトオーダー）として正しく読む。さらに `$msg_length` の上限チェックを追加して異常値を拒否する。
...
**問題点**: ネットワークから受け取った `data` はビッグエンディアンで書かれているのに `readUInt32LE(0)` でリトルエンディアン解釈をしている。`[0x00, 0x00, 0x01, 0x00]`（= 256）が `0x00010000 = 65536` として読まれ、`data.slice(4, 65540)` が呼ばれる。
...
`readUInt32BE(0)` でビッグエンディアンとして正しく読む。`msgLen` の上限チェックと実際のバッファ長との整合確認も行う。
...
**問題点**: `'<I'`（リトルエンディアン）でネットワークの長さフィールドを読んでいる。`b'\x00\x00\x01\x00'`（ビッグエンディアンで 256）が `0x00010000 = 65536` と誤読される。
...
`'!I'`（ネットワークバイトオーダー = ビッグエンディアン）で正しく読む。`socket.ntohl()` を使う方法もある。
...
**誤解 1: 「ビッグエンディアンが正しくて、リトルエンディアンは間違い」**
...
**誤解 3: 「ARM はビッグエンディアン」**
ARM は「バイエンディアン（Bi-Endian）」アーキテクチャで、ビッグ・リトルどちらも動作できる。しかし現代の ARM デバイス（スマートフォン・Raspberry Pi・Apple M シリーズ）では OS がリトルエンディアンモードで動作するよう設定している。「ARM = ビッグ」は古い知識だ。

**誤解 4: 「ネットワークパケットの中身をそのまま整数として使えば大丈夫」**
...
**誤解 5: 「pwntools の p32() は常にリトルエンディアンでパックする」**
