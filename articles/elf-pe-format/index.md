...
この図は ELF ファイルの主要な構造要素と各セクションの対応関係を示している。見るポイントは、ヘッダが「どこに何があるか」を指示し、プログラムヘッダ表が OS のメモリロードに使われ、セクションヘッダ表がリンカや解析ツールに使われる二重構造だ。特に `.dynamic` と `.init_array` はマルウェアがコードを仕込む対象として注目する。
...
> #### 💡 ワンポイントアドバイス
> - `file` と `strings` の 2 コマンドだけで驚くほどの情報が取れる。バイナリを渡されたらまずこの 2 つを実行する習慣をつけよう。情報収集の基本動作だ。
> - セクション名（`.text` / `.plt` / `.got` / `.init_array`）は「住所」のようなもの。攻撃ターゲットは大体 GOT と `.init_array` に絞られる。`readelf -S` で確認しよう。
> - ELF と PE は「同じ問題を異なる方法で解いた設計」。Linux 側を深く理解すると Windows 側の習得が大幅に速くなる。共通するセクション概念から攻めよう。
...
この図は「GOT エントリをバッファオーバーフローで上書きして任意コード実行に至る攻撃」と「RELRO 有効時にブロックされる経路」を示している。学習ポイントは GOT が書き込み可能かどうかが攻撃成否を左右すること、そして Full RELRO がその書き込みを禁止する位置にあることだ。
...
**問題点**: `$_FILES['file']['type']` は HTTP リクエスト内のフィールドであり、攻撃者が `curl` でリクエストを改ざんして `Content-Type: text/plain` を付けながら ELF ファイルを送ることで MIME チェックを突破できる。さらに `?exec=1` で `chmod()` が実行権限を付与し、`shell_exec()` がその ELF ファイルを直接実行する。
...
静的リンクすると PLT/GOT が存在しないため GOT ハイジャックはできなくなる。ただしファイルサイズが大幅に増加し、ASLR の効果が下がるデメリットがある。また `LD_PRELOAD` による共有ライブラリ差し込みは防げるが、`.init_array` のような他の攻撃ベクターは残る。

**誤解 3: 「SUID root ファイルは危険だからすべて削除すればいい」**
`sudo`・`ping`・`passwd` など多くのシステムツールが SUID を必要とする。削除すると OS の正常動作に支障が出る。正しい対策は SUID ファイルの棚卸し・最小権限の原則の適用・カーネル/ライブラリのアップデートだ。
...
**誤解 5: 「`strings` で見つからない文字列はファイルに存在しない」**
