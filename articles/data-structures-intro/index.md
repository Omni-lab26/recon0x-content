...
> **本記事で前提とする用語の超ざっくり整理**
> - **配列（Array）**: 連続したメモリ領域に同じ型のデータを並べたデータ構造。インデックス（番号）でO(1)アクセスできる。
> - **連結リスト（Linked List）**: 各要素が「次の要素へのポインタ」を持ち、鎖のようにつながるデータ構造。メモリが連続している必要はない。
> - **ハッシュテーブル（Hash Table）**: キーをハッシュ関数で整数に変換してインデックスにするデータ構造。平均O(1)で検索できる。辞書型・連想配列とも呼ばれる。
...
> - **ハッシュフラッディング**: 同じハッシュ値になる大量のキーをハッシュテーブルに挿入し、O(1)の性能をO(n)まで劣化させるDoS攻撃。
...
データ構造は「プログラムがデータをどこにどう置くか」を決める設計だ。これを理解しないと、以下の問いに答えられない。

- **なぜバッファオーバーフローで任意コード実行ができるのか？** → 配列がメモリ上で連続していて、隣に戻りアドレスがあるから。
- **なぜハッシュテーブルへの大量リクエストでサーバーが落ちるのか？** → ハッシュ衝突が連鎖してO(1)がO(n²)に劣化するから。
...
- **ペネトレーションテスト**: ハッシュフラッディングでWebアプリのパースに負荷をかける
...
この図は配列がメモリ上でどのように配置されるかと、インデックスアクセスの計算方法を示している。`arr-0` から `arr-n` は各インデックスの要素を表し、`arr-n` は途中の要素を省略した最後の要素を意味する（実際の配列名ではない）。
...
> **O(1)の意味**: 「定数時間」。配列の要素数が 10 個でも 100 万個でも、インデックスアクセスの時間は変わらない。アドレス計算一発で済むから。
...
この図は単方向連結リストのノード間の参照関係と、末尾を示す NULL ポインタを示している。
...
ハッシュテーブルはキーをハッシュ関数で整数に変換し、その値を配列のインデックスとして使うデータ構造だ。平均的にO(1)でのデータ検索・追加・削除を実現する。

この図はキーからハッシュ値を経て配列インデックスへ至る変換過程と、異なるキーが同じインデックスに衝突した場合にチェーン（連結リスト）で解決される様子を示している。
...
**セキュリティ上の問題点 — ハッシュフラッディング**:

全キーが同じハッシュ値になるよう細工された大量のキーを挿入すると、全てが同一のバケットに連結リストとしてチェーンされる。すると本来O(1)の検索がO(n)になり、n 件挿入で検索コストがO(n²)に達し、CPU が枯渇する。
...
**問題点**: `?id=-1` や `?id=9999` のように範囲外のインデックスを送ると PHP は `NULL` を返すか警告を出す。より深刻な例では、他のユーザーのデータが詰まった配列に対して想定外のインデックスでアクセスし、情報が漏洩することがある。
...
**問題点**: 攻撃者が数万個のキーを持つ JSON を POST すると、大量のキー処理や巨大 JSON のパースにより CPU・メモリ消費が急増する。また単純にキー数が多いだけでもメモリを大量消費し、サービス妨害（DoS）につながる。
...
キー数制限・ボディサイズ制限の両方を組み合わせることで、ハッシュフラッディングの前提となる大量データの投入を防ぐ。
...
**問題点**: Python では `store[1]` と `store["1"]` は**異なるキー**だが、`store[True]` と `store[1]` は**同じキー**だ（`True == 1` が成立するため）。また`key` を全てユーザー入力の文字列として受け付けると、内部の予約キーや管理用キーを上書きされる可能性がある。
...
**誤解 1: 「Python や JavaScript に配列の境界チェックは不要」**
Python のリストや JavaScript の配列はインデックス範囲外アクセスで例外を投げてくれるが、ユーザー入力のインデックスをそのまま使うと負のインデックスで末尾からアクセスされる（Python のスライス仕様）。`arr[-1]` は最後の要素だ。想定外のアクセスを防ぐには明示的な範囲チェックが必要だ。

**誤解 2: 「ハッシュテーブルは常に O(1)」**
...
**誤解 4: 「free() したポインタは自動的に NULL になる」**
C言語では `free(p)` を呼んでもポインタ `p` の値（アドレス）は変わらない。これを「ダングリングポインタ」と呼ぶ。`free` 後に明示的に `p = NULL` を代入する習慣をつけないと use-after-free や double-free の原因になる。

**誤解 5: 「ハッシュ値が等しければキーも等しい」**
ハッシュ値は固定長なので衝突（異なるキーが同じハッシュ値になる）は必ず起きる。ハッシュテーブルの実装は「ハッシュ値が等しい」かつ「`==` で等しい」の両方を確認してからキーが一致したと判断する。ハッシュ値だけで一致を確認するコードは誤りだ。
