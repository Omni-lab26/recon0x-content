...
`printf("hello")` を書いたとき、そのコードはどうやって libc の `printf` の実装にたどり着くのか。静的リンクなら答えは単純だ——バイナリ内に `printf` のコピーが埋め込まれている。だが動的リンクでは、`printf` の「住所」は実行時まで確定しない。

この「住所の解決」を担うのが PLT・GOT・ld.so が連携して動作する仕組みだ。

セキュリティの文脈で動的リンクの知識が必要になる場面を挙げる。
...
この図はライブラリを探す 4 段階の優先順位を示している。上位ほど高優先で、攻撃者が制御できる経路ほど危険だ。
...
この図は「プログラムが外部関数（例: `printf`）を初めて呼ぶとき PLT/GOT が動く仕組み」と「2 回目以降は GOT から直接呼ばれる省略経路」、そして「シンボルが見つからないときのロード失敗」を示している。
...
この図では攻撃者が環境変数 `LD_PRELOAD` に悪意ある `.so` のパスを書き込んだ状況を表している。`ld.so` は通常のライブラリより先に `LD_PRELOAD` で指定された `.so` をロードするため、同名の関数が上書きされる。SUID ファイルでは `ld.so` がセキュア実行モードに入り `LD_PRELOAD` を無視する分岐も示している。
...
**問題点**: 攻撃者が `?lib=evil.so&cmd=id` を送ると、`LD_PRELOAD=/tmp/evil.so` が設定された状態で `mail()` が子プロセス（`sendmail`）を起動する。`evil.so` は `__attribute__((constructor))` でロード時に `EVIL_CMD` の内容を実行し、結果を `/tmp/evil_output.txt` に書き出す。
...
Web アプリケーションから `putenv()` で `LD_PRELOAD` を操作する設計は根本的に避ける。`php.ini` の `disable_functions` に `putenv,mail,system,exec,popen,proc_open,shell_exec` を追加して外部プロセス起動と環境変数操作を禁止することも重要だ。
...
**問題点**: ユーザーが `?module=./../../tmp/evil` のような相対パスを指定すると、任意の JavaScript ファイルや `.node` ネイティブアドオンが読み込まれる。`.node` ファイルが ELF 形式のため、ロードされた瞬間に `__attribute__((constructor))` のコードが実行される。`node_modules` の解決とパス指定による読み込みは別の仕組みだが、どちらも `require()` が受け入れるため危険だ。
...
許可リスト方式で受け入れるモジュール名を固定する。`./`・`../`・絶対パスを含む入力はすべて拒否する。依存モジュールは `package-lock.json` で整合性を管理し、`npm audit` で定期確認する。
...
**問題点**: `?lib_path=/tmp/evil` を送ると `/tmp/evil` に置かれた悪意ある `.so` が `/usr/local/bin/analyzer` より先に読み込まれる。`analyzer` が依存する `libssl.so` などの正規ライブラリを同名で `/tmp/evil/` に配置しておくだけで関数置き換えが成立する。
...
**誤解 1: 「LD_PRELOAD はデバッグ専用で攻撃には使えない」**
...
**誤解 2: 「GOT は実行中に書き換えられない」**
Full RELRO が有効な場合は起動直後に読み取り専用になるが、Partial RELRO（デフォルト）では実行中も書き込み可能なまま。`checksec` で確認せずに「GOT は安全」と思い込むのは危険だ。

**誤解 3: 「ld.so は OS の一部だから脆弱性はない」**
`CVE-2023-4911`（Looney Tunables）は glibc の `ld.so` 自体のバッファオーバーフローで、ローカルユーザーが root 権限昇格できた重大な脆弱性だ。`ld.so` もパッチが必要な通常のソフトウェアだ。

**誤解 4: 「静的リンクすれば動的リンク系の攻撃はすべて防げる」**
静的リンクすると `LD_PRELOAD`・`LD_LIBRARY_PATH` は効かなくなる。ただし静的リンクでも PIE と ASLR は利用可能であり、アドレスランダム化の恩恵は受けられる。また静的リンク固有の攻撃面やライブラリ脆弱性の固定化などのデメリットもある。

**誤解 5: 「PHP の putenv() は子プロセスに影響しない」**
