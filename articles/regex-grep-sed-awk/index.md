...
セキュリティエンジニアが `grep`・`sed`・`awk` を使う場面は多い。
...
正規表現の学習順序と各要素の階層関係を示す。基礎的な「アンカー」「文字クラス」「量指定子」を理解してから、「グループ」「後方参照」「先読み」という発展機能へ進むのが推奨ルートだ。
...
この図は 3 つのツールをユースケース別に分類している。「選ぶ」「変換する」「集計する」という役割の違いを把握することが使い分けの出発点だ。
...
この図は「ReDoS の悪意ある入力によって正規表現エンジンが CPU を枯渇させる攻撃フロー」と「タイムアウト設定によって防御される経路」を示している。
...
**問題点**: `([\w.]+@[\w.]+)+` はネストした量指定子を持つ。攻撃者が `aaaaaaaaaaaaa@aaaaaaaaaaaaa@` のような無効なメールアドレスを送ると、エンジンが指数関数的な試行を行いサーバーが数秒〜数分間フリーズする。
...
長さ制限を先に行うことで入力の大きさを制限し、正規表現の試行回数の上限を抑える。
...
**問題点**:
1. **正規表現インジェクション**: `?term=(a+)+%24` のような ReDoS ペイロードを送られると CPU が枯渇する
2. **情報漏洩**: `?term=password` で全ログのパスワード行が返る
3. **正規表現エラー**: `?term=[invalid` のような不正パターンでサーバーが例外を投げる
...
**問題点**: `?pattern=(a%2B)%2B&input=aaaaaaaaaaaaaab`（`(a+)+` と長い `a...b`）を送ると ReDoS が発生する。Python の `re` モジュールはデフォルトでタイムアウトなしで動作するため、処理が返ってこなくなる。
...
ユーザーが正規表現パターンを指定できる設計を避け、許可リスト方式でパターン名から選ぶ。各パターンは事前に安全性を確認したうえでコンパイル済みオブジェクトとして用意する。
...
**誤解 2: 「`.*` は任意の文字列にマッチするから万能」**
`.*` は「改行を除く任意の文字の 0 回以上の繰り返し」だ。複数行テキストの全体にマッチさせたい場合は `re.DOTALL` フラグ（Python）や `/s` フラグを使う必要がある。これを知らずに書くとセキュリティチェックが改行を含む入力で回避される。

**誤解 3: 「grep の `|` は OS のパイプ」**
`grep -E "cat|dog"` の `|` は正規表現の「または」演算子で、「grep の出力を別コマンドに渡す」OS のパイプ（`|`）とは別物だ。コマンドを組み合わせるパイプは `grep "error" log | sort` のように、コマンドとコマンドの間に置く。

**誤解 4: 「awk はアーカイブ展開ツール」**
...
**誤解 5: 「正規表現インジェクションは RCE にならないから低リスク」**
