...
> #### 💡 ワンポイントアドバイス
> - `strace ls` を実際に実行してみよう。「たった 1 コマンドでこれだけの syscall が呼ばれるのか」という驚きがOS理解の入口になる。まず動かして感覚をつかむのが最短ルート。
> - syscall 番号の暗記は不要。「`execve` = プログラム起動」「`mmap` = メモリ確保」「`write` = 出力」という役割を覚えれば、strace の出力が読めるようになる。
> - `shell_exec()`・`exec()`・`subprocess.run(shell=True)` を見かけたら「ユーザー入力が渡っていないか」を反射的に確認する習慣をつけよう。これだけでコマンドインジェクションの発見率が大幅に上がる。
