...
この問いに即答できないなら、この記事が助けになる。答えはシンプルだ——**16進数 `1F` は 10進数の 31**。2進数・16進数・ビット演算の感覚があるかどうかで、セキュリティの現場で見えるものが変わる。

具体的に挙げると：

- Nmap のスキャン結果に `0x1F` が出てきたとき、即座に `31` と読めるか
- シェルコードで `\x90` が x86 系 CPU で NOP 命令として使われ、NOP sled の構成要素になると理解できるか
- バッファオーバーフローのオフセット計算でビット境界がずれたときに気づけるか

これらはプログラミングの問題ではなく、コンピューターが何をしているかを直接読む能力の問題だ。
...
10進数の値が左から右へ「2進数 → 4ビット単位 → 16進数」と変換されていく流れだ。この「4ビット = 16進数1桁」の対応関係こそが、メモリダンプを読む速度を決める。
...
1つの演算子から複数の操作が派生している構造が見える。どれを使うかで「ビットを消す・立てる・反転する・ずらす」のどれを行うかが決まる。

**計算量まとめ**

各演算の記号と用途を整理する。

- **AND `&`**: 両方が 1 のときだけ 1。「両方の条件をくぐり抜けたものだけ通す」フィルター。特定ビットの抽出・マスク処理に使う。
- **OR `｜`**: どちらかが 1 なら 1。「どちらかに当てはまれば OK」のスタンプ。特定ビットのセット・フラグ追加に使う。
- **XOR `^`**: 違うときだけ 1。「差分検出」「同じ鍵で 2回かけると元に戻る」性質があり、ビット反転・簡易暗号化に使われる。
- **NOT `~`**: 0 と 1 をすべて反転。全ビット反転・マスク生成に使う。
- **左シフト `<<`**: ビット列を左にずらす。空いた右側には 0 が入る。多くの環境で 2のn乗倍相当（オーバーフローに注意）。
- **右シフト `>>`**: ビット列を右にずらす。あふれた右端のビットは捨てられる。多くの環境で 2のn乗除算相当（符号付き整数では実装差異あり）。

**ビット演算の弱点 — 実装ミスによる権限バイパス**

演算子の選択を誤ると、意図しないビットまで通過させてしまう。「フィルターのつもりが全通し」になるパターンが後述する脆弱なコード例の核心だ。
...
3種類の権限がそれぞれ独立したビットとして扱われているのがポイントだ。この「ビットの独立性」が AND による精密な権限チェックを可能にする一方で、チェック実装を誤ると意図しない権限が通ってしまう。

**計算量まとめ**

- **アクセス（AND でビット取得）**: O(1)。整数同士の演算なので即座に結果が出る。
- **権限の追加（OR）**: O(1)。同様に即座。
- **権限の削除（AND NOT）**: O(1)。同様に即座。

**パーミッション管理の弱点 — ビット検査の抜け穴**

AND 演算でビットを取り出すコードは正しく書けば安全だが、「権限値をユーザー入力から受け取る」という設計ミスが重なると即座に崩壊する。
...
**「ビット演算はアセンブリを書くときだけ使うもの」**
権限フラグ・ネットワークマスク・RGB カラー値や画像フォーマット内の色表現・HTTP ヘッダーのフラグなど、**高レベルなウェブアプリ開発でも日常的に使われている**。知らずに書いていたコードが実はビット演算だったというケースは多い。

**「XOR 暗号は鍵を知らないと解読できない」**
同じ鍵を 2回以上使えば **Two-time pad 攻撃**で解読できる。ワンタイムパッドが安全な理由は「真の乱数」を「絶対に1回しか使わない」ことが前提になっているからだ。この条件を満たせない設計では XOR 暗号を使ってはいけない。
...
**どこが問題か**: 権限値をクエリパラメータからそのまま受け取っているのが根本的な欠陥だ。攻撃者がブラウザで `?perm=128`（= `0b10000000`）を送信するだけで `hasPermission` の AND チェックが通り、管理者パネルに到達できる。「ビット演算で安全にチェックしている」と思っていても、**チェック対象の入力値を攻撃者が自由に操作できる**時点でチェックは無意味になる。
...
**どこが問題か**: `| 0` は下位 32ビットだけを残して切り捨てる。`4294967314` は 16進で `0x100000012` であり、`| 0` 後は `0x12` = 18 になる。本来 18 ではない値が「18 として認証ロジックを通過」してしまうため、**年齢制限のかかったコンテンツや購入フローを任意にバイパスできる**。JavaScript の Number は IEEE754 倍精度浮動小数であり、これは典型的な整数オーバーフローとは異なる性質の問題だ。
...
**どこが問題か**: 同一の鍵で 2つのメッセージを暗号化すると `enc1 XOR enc2 = msg1 XOR msg2` が成立する。**鍵を一切知らない攻撃者でも平文の関係式が露出し、既知平文が1つでもあれば残りのメッセージを完全復元できる**。
...
ポイントは「権限値は DB から取る」「ユーザー入力（クエリパラメータ・Cookie・隠しフォームなど）は信用しない」の 2点だ。この「**信頼できるソースからのみ権限を取得する**」原則は、ビット演算に限らずあらゆる認証ロジックの基本になる。
...
32ビット符号付き整数の最大値は `2,147,483,647`（`0x7FFFFFFF`）だ。これを超えた値が入力されうる状況では、言語の型変換を信頼せず上限チェックを入れる。**「型変換に任せる前にはじく」のが、ToInt32 系バイパスへの根本的な対策だ。**
...
**「標準ライブラリの暗号を使い、独自実装しない」**——これが XOR 鍵再利用問題を含むあらゆる暗号実装ミスを防ぐ最も確実な方法だ。
