...
「バッファオーバーフローでなぜ任意のコードが実行できるのか？」

この問いに即答できないなら、この記事が助けになる。答えはシンプルだ——**スタックにローカル変数と戻りアドレスが隣り合っていて、境界チェックがなければ溢れたデータが戻りアドレスを上書きできるから**。メモリモデルを知れば、CVE レポートに「スタックオーバーフロー」「ヒープコラプション」と書かれたとき何が起きているかを感覚的に追えるようになる。

具体的に挙げると：

- CTF の Pwn カテゴリで「ヒープ上でシェルコードを実行する」と聞いたとき、ヒープとスタックの違いが分からなければ攻撃のメカニズムを追えない。
- 実際の CVE を読むとき、「スタックバッファが溢れた」「ヒープの解放済み領域を再参照した」というパターンを理解しているかどうかが、レポートを読めるかの分岐点になる。
...
スタックが最も高番地、テキストが最も低番地に置かれる。そしてヒープは低アドレスから高アドレス方向へ、スタックは高アドレスから低アドレス方向へ成長する——この「向かい合う構造」が、どちらかが溢れたとき隣領域を破壊しやすい理由だ。
...
**メモリレイアウトの弱点 — セグメント間の隣接と境界突破**

スタック上のバッファとその隣の戻りアドレスは連続したメモリ上に並んでいる。境界チェックがなければ、バッファに想定を超えるデータを書くだけで戻りアドレスを上書きできる——これがバッファオーバーフローの本質だ。
...
境界チェックがあるかどうかで「正常終了」と「任意コード実行」に分岐する——この 1 点がスタックバッファオーバーフロー防御の核心だ。スタックカナリアはこの分岐を「戻りアドレスに到達する前に検知」することで防ぐ。
...
**「Python や PHP はメモリ安全だからバッファオーバーフローは関係ない」**
...
**どこが問題か**: `unserialize()` はユーザー入力から任意クラスのオブジェクトを生成できる。攻撃者は `$path` を `/etc/passwd` に書き換えた Config オブジェクトをシリアライズして URL パラメータに渡すだけで任意ファイルを読める。さらに、アプリケーション内に `__wakeup()` や `__destruct()` マジックメソッドを持つクラスが存在する場合、**POP Chain によるコード実行へ発展する可能性がある**。
...
`unserialize()` をユーザー入力に対して使わない。どうしても使う場合は第 2 引数で `['allowed_classes' => false]` を指定してクラスの復元を禁止する。**「JSON を使い、unserialize は原則禁止」というルールが PHP のオブジェクト注入全般への根本的な対策だ。**
...
**どこが問題か**: クライアントが `http://server/1024` にアクセスするだけで、1024 バイトの未初期化ヒープメモリがレスポンスとして返ってくる。**このデータには直前の HTTP リクエスト・セッショントークン・パスワード断片が含まれる可能性があり、攻撃者はリクエストサイズを調整するだけで任意量のヒープメモリを読み出せる**。
...
`Buffer.alloc()` を使えばゼロ初期化されるため情報漏洩は起きない。サイズの上限チェックも必ず加える。**`allocUnsafe` を `alloc` に変えるだけで、ヒープ上の機密データが外部に漏れるリスクを排除できる。**
...
**どこが問題か**: `ctypes.string_at()` は Python のサンドボックスを無視して任意アドレスのメモリを読む。Web サーバーがユーザー入力をアドレスとして受け付けて `ctypes.string_at()` に渡すような実装があれば、**攻撃者はアドレスを変えるだけでヒープ全体を読み出せる**。セッション情報・パスワード・暗号鍵が漏洩するリスクがある。

**防御策:** Web アプリケーション層で `ctypes` を使う必要は通常ない。`ctypes` が必要な処理は独立したサービスに分離し、アドレスをユーザー入力から受け取る設計は絶対に避ける。**「ユーザー入力が ctypes に流れる経路を設計段階で排除する」ことが唯一の根本的な対策だ。**
